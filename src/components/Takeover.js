...
export const Takeover = ({callback, time}) => {
  ...
  useEffect(() => {
    ...
    something.forEach((el, i) => {
        result1.push(<li className="result-position">{i}</li>)
        result1.push(<li className="result-initials">{el.initials}</li>)
        result1.push(<li className="result-time">{el.time}</li>)
    })
